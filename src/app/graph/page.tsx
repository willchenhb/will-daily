...
function GraphLoader({
  ...
  useEffect(() => {
    ...
    for (const node of filteredNodes) {
      ...
      const isSearchMatch = searchTerm && node.title.toLowerCase().includes(searchTerm.toLowerCase())
      ...
      graph.addNode(String(node.id), {
        ...
        borderColor: isSearchMatch ? '#fbbf24' : undefined,
      })
    }
        ...
        try {
          graph.addEdge(String(edge.nodeAId), String(edge.nodeBId), {
            ...
            color: selectedNodeId
              ? (edge.nodeAId === selectedNodeId || edge.nodeBId === selectedNodeId
                  ? '#ef4444'
                  : '#e5e7eb')
              : '#d1d5db',
          })
          ...
          {Object.entries(TYPE_LABELS).map(([type, label]) => (
            ...
            >
              <span
                ...
                style={{ backgroundColor: filters[type] ? TYPE_COLORS[type] : '#d1d5db' }}
              />
              {label}
            </button>
          ))}
        ...
        {selectedNode && (
          <div className="w-[320px] border-l border-gray-100 bg-white overflow-y-auto flex-shrink-0">
            <div className="p-4">
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{ backgroundColor: TYPE_COLORS[selectedNode.sourceType] }}
                  />
                  <span className="text-[11px] text-gray-400">
                    {TYPE_LABELS[selectedNode.sourceType]}
                  </span>
                </div>
                <button
              ...
              {/* Title */}
              <h3 className="text-[15px] font-medium text-gray-800 mb-2 leading-snug">
                {selectedNode.title}
              </h3>
              ...
              {/* Link to source */}
              <a
                href={TYPE_LINKS[selectedNode.sourceType]?.(selectedNode.sourceId)}
                className="inline-block text-[12px] text-blue-500 hover:text-blue-600 mb-4"
              >
                查看原文 &rarr;
              </a>
              ...
              {connectedNodes.length > 0 && (
                <div>
                  <h4 className="text-[12px] font-medium text-gray-600 mb-2">
                    关联内容 ({connectedNodes.length})
                  </h4>
                      ...
                      >
                        <div className="flex items-center gap-1.5 mb-1">
                          <span
                            ...
                            style={{ backgroundColor: TYPE_COLORS[node.sourceType] }}
                          />
                          <span className="text-[11px] text-gray-400">
                            {TYPE_LABELS[node.sourceType]}
                          </span>
